...
Script installer otomatis (all-in-one) untuk membangun VPS multi-protokol: **X-ray (VMess/VLESS/Trojan)**, **SSH WebSocket + Dropbear 2019**, dan **ZIVPN (UDP)** — lengkap dengan panel menu manajemen akun, notifikasi Telegram ke admin, auto-expired, limit IP, dan quota bandwidth.

```bash
bash <(curl -Ls https://raw.githubusercontent.com/tendostore/AUTO-SCRIPT-TENDO-STORE-AIO/main/install.sh)
```

> Edisi **Admin Notif Only** — semua notifikasi (akun baru, login, multi-login, kuota habis, backup) dikirim ke Telegram admin. Tidak ada fitur bot client untuk reseller di edisi ini.
...
- **Notifikasi Telegram ke admin**: akun baru dibuat, user login, multi-login terdeteksi, kuota habis, sampai auto-backup konfigurasi VPS terjadwal.
...
Kalau kena rate limit GitHub (`429: Too Many Requests`), download dulu baru jalankan:

```bash
curl -Ls https://raw.githubusercontent.com/tendostore/AUTO-SCRIPT-TENDO-STORE-AIO/main/install.sh -o install.sh
bash install.sh
```

Saat instalasi berjalan, kamu akan diminta memilih **jenis domain** — domain random otomatis atau domain milik sendiri (pastikan A record sudah diarahkan ke IP VPS jika pilih manual). Tunggu sampai proses selesai, lalu ketik `menu` untuk membuka panel.
...
| SSH WS (TLS via stunnel, backend Dropbear) | 8443 |
| SSH WS (alternatif, backend Dropbear) | 8080, 2082, 2083, 8880 |
...
- Setup bot Telegram admin (notifikasi login, notifikasi backup)
- Reboot terjadwal, rebuild config, cek layanan

---

## 🤖 Setup Notifikasi Telegram (Admin)
...
4. Atur jadwal notifikasi login (menit/jam) dan jadwal auto-backup sesuai kebutuhan.

---

## ⚠️ Catatan Keamanan

- Ganti kredensial API (Cloudflare token, dsb.) di dalam script dengan milikmu sendiri sebelum dipakai untuk produksi/reseller — **jangan gunakan token/API key bawaan contoh**, dan pastikan API key/token bersifat *scoped* (bukan Global API Key) supaya kalau bocor dampaknya terbatas.
...
- Selalu rotate Bot Token/API key jika script pernah dibagikan/bocor ke pihak lain.
